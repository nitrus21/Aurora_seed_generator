...
FW = WEB / 'firmware'
CATALOG = {
    'cyd-1.9.3': ('manifest.json', '1.9.3', 0, 0x1000, None),
...
def main():
    ...
    assert set(page.options) == set(CATALOG) and len(page.options) == 4
    ...
    for version, device, images in (
        ('1.9.3', 'CYD', ['aurora-1.9.3-esp32-2432s028r.factory.bin']),
