...
$variants = @(
    @{ Profile='waveshare-p4-rev1'; Id='p4-rev1-2.0.5'; Manifest='manifests\p4-rev1-2.0.5.json'; Image='aurora-2.0.5-esp32-p4-rev1.factory.bin' },
    @{ Profile='waveshare-p4'; Id='p4-rev3-2.0.5'; Manifest='manifests\p4-rev3-2.0.5.json'; Image='aurora-2.0.5-esp32-p4-rev3.factory.bin' }
)
foreach ($variant in $variants) {
    $manifest = Get-Content -Raw -LiteralPath (Join-Path $web $variant.Manifest) | ConvertFrom-Json
    Assert-P4 ($manifest.version -eq '2.0.5' -and $manifest.builds.Count -eq 1) "Manifest invalide: $($variant.Id)"
...
Write-Output 'PASS: P4 2.0.5 rev1/rev3, manifests, images, SHA-256, notes and Web Flasher'
