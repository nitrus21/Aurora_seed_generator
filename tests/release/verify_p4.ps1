...
Assert-P4 ($versionMatch.Success -and $versionMatch.Groups[1].Value -eq '2.0.9') 'P4 source must match the newest published image.'
...
$variants = @(
    @{ Profile='waveshare-p4-rev1'; Id='p4-rev1-2.0.9'; Manifest='manifests\p4-rev1-2.0.9.json'; Image='aurora-2.0.9-esp32-p4-rev1.factory.bin' },
    @{ Profile='waveshare-p4'; Id='p4-rev3-2.0.9'; Manifest='manifests\p4-rev3-2.0.9.json'; Image='aurora-2.0.9-esp32-p4-rev3.factory.bin' }
)
foreach ($variant in $variants) {
    $manifest = Get-Content -Raw -LiteralPath (Join-Path $web $variant.Manifest) | ConvertFrom-Json
    Assert-P4 ($manifest.version -eq '2.0.9' -and $manifest.builds.Count -eq 1) "Manifest invalide: $($variant.Id)"
    ...
    & python (Join-Path $PSScriptRoot '..\crypto\verify_p4_images.py') --profile $variant.Profile --factory $image --version 2.0.9
    Assert-P4 ($LASTEXITCODE -eq 0) "Image P4 invalide: $($variant.Profile)"
}
Assert-P4 (Test-Path -LiteralPath (Join-Path $web 'releases\2.0.9.md')) 'Notes 2.0.9 absentes.'
...
Write-Output 'PASS: P4 2.0.9 rev1/rev3, manifests, images, SHA-256, notes and Web Flasher'
